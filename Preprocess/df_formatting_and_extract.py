...
class CardCycleProcessor:
    ...
    def card_cycle_processing(self, date_range: list, col: str, method: str, name: str,
        ...
        for i in tqdm(range(1, len(date_range)), desc=name):
            if method == 'ratio':
                mapping = self.df[self.df['授權日期'].isin(previous_dates)].groupby(
                    '交易卡號')[col].value_counts(normalize=True).to_dict()
            else:
                mapping = self.df[self.df['授權日期'].isin(previous_dates)].groupby('交易卡號')[
                    col].agg(method).to_dict()

            if post_processing is None:
                self.df.loc[self.df['授權日期'].isin(date_range[i]), name] = self.df[self.df['授權日期'].isin(
                    date_range[i])]['交易卡號'].map(mapping)
            else:
                self.df.loc[self.df['授權日期'].isin(date_range[i]), name] = self.df[self.df['授權日期'].isin(
                    date_range[i])].apply(lambda x: post_processing(x, mapping, col), axis=1)
...
def main():
    ...
    all_data['高金額'] = (all_data['轉換後交易金額'] > train[train['盜刷註記']
                       == 0]['轉換後交易金額'].quantile(q=.80)).astype('int8')
    ...
    all_data['交易類別_交易型態'] = all_data['交易類別'].astype(
        'str')+"_"+all_data['交易型態'].astype('str')

    # 整體資料在各分類的佔比
    previous_dates = date_list[0].copy()
    for col in ['商戶類別代碼', '消費城市', '收單行代碼', '特店代號']:
        for i in tqdm(range(1, len(date_list)), desc=f'{col}整體資料佔比'):
            mapping = all_data[all_data['授權日期'].isin(
                previous_dates)][col].value_counts(normalize=True).to_dict()
            all_data.loc[all_data['授權日期'].isin(date_list[i]), f'{
                col}消費總比例'] = all_data[all_data['授權日期'].isin(date_list[i])][col].map(mapping)
    ...
    for i in tqdm(range(1, len(date_list)), desc='每週期平均交易數'):
        mapping = all_data[all_data['授權日期'].isin(previous_dates)].groupby(
            ['交易卡號', 'loading_cycle'])['盜刷註記'].count().groupby(level=0).mean().to_dict()
        all_data.loc[all_data['授權日期'].isin(date_list[i]), '每週期平均交易數'] = all_data[all_data['授權日期'].isin(
            date_list[i])]['交易卡號'].map(mapping)
        previous_dates += date_list[i]
    all_data['每週期平均交易數'].fillna(0, inplace=True)
    processor = CardCycleProcessor(all_data)

    all_data = processor.card_cycle_processing(
        date_list, '盜刷註記', 'mean', '盜刷比例')
    all_data = processor.card_cycle_processing(
        date_list, '轉換後交易金額', 'mean', '個人平均消費金額倍率', CardCycleProcessor.spending_ratio)
    ...
    for col in ['時段', '網路交易註記', '商戶類別代碼', '消費城市', '收單行代碼', '特店代號']:
        all_data = processor.card_cycle_processing(date_list, col, 'ratio', f'卡號在{
                                                   col}的比例', CardCycleProcessor.multi_key_mapping)

    # 標註是否為新消費者(前面的cycle沒有看到的卡號)
    card_list = set()
    for i in tqdm(range(0, len(date_list)), desc='新消費者'):
        # update all_data info first
        all_data.loc[all_data['授權日期'].isin(date_list[i]), '新消費者'] = all_data[all_data['授權日期'].isin(
            date_list[i])].apply(lambda x: x['交易卡號'] not in card_list, axis=1)
        # update card_df
        new_data = all_data[all_data['授權日期'].isin(date_list[i])]
        new_cards = [card for card in new_data['交易卡號'].unique()
    ...
    card_df = pd.DataFrame(columns=[
                           '交易卡號', '交易總次數', '網路消費次數', '國內消費次數', '交易週期數', '可交易週期數', '網路交易behavior', '國內消費behavior'])
    card_list = set()
    for i in tqdm(range(0, len(date_list))):
        # update all_data info first
        card_df.set_index('交易卡號', inplace=True)
        all_data.loc[all_data['授權日期'].isin(date_list[i]), '是否符合網路消費習慣'] = all_data[all_data['授權日期'].isin(
            date_list[i])].apply(lambda x: check_behavior(x, card_df, card_list, '網路交易behavior', '網路交易註記'), axis=1)
        all_data.loc[all_data['授權日期'].isin(date_list[i]), '是否符合國內外消費習慣'] = all_data[all_data['授權日期'].isin(
            date_list[i])].apply(lambda x: check_behavior(x, card_df, card_list, '國內消費behavior', '消費地國別'), axis=1)
        all_data.loc[all_data['授權日期'].isin(date_list[i]), '國內消費比例'] = all_data[all_data['授權日期'].isin(
            date_list[i])].apply(lambda x: query_ratio(x, card_df, card_list, '國內消費比例'), axis=1)
        all_data.loc[all_data['授權日期'].isin(date_list[i]), '消費頻率_週期'] = all_data[all_data['授權日期'].isin(
            date_list[i])].apply(lambda x: query_ratio(x, card_df, card_list, '消費頻率_週期'), axis=1)
        ...
        new_cards = [card for card in new_data['交易卡號'].unique()
                     if card not in card_list]
        card_df = pd.concat(
            [card_df, pd.DataFrame(new_cards, columns=['交易卡號'])])
        ...
        card_df['交易總次數'] = card_df.apply(
            lambda x: update(x, '交易總次數', mapping, 'sum'), axis=1)
        card_df['交易週期數'] = card_df.apply(
            lambda x: update(x, '交易週期數', mapping, 'count'), axis=1)
        mapping = new_data.groupby('交易卡號')['網路交易註記'].sum().to_dict()
        card_df['網路消費次數'] = card_df.apply(
            lambda x: update(x, '網路消費次數', mapping, 'sum'), axis=1)
        mapping = new_data[new_data['消費地國別'] == 0].groupby(
            '交易卡號')['轉換後交易金額'].count().to_dict()
        card_df['國內消費次數'] = card_df.apply(
            lambda x: update(x, '國內消費次數', mapping, 'sum'), axis=1)
        card_df['國內消費比例'] = card_df['國內消費次數']/card_df['交易總次數']

        card_df.loc[card_df['網路消費次數'] /
                    card_df['交易總次數'] > 0.7, '網路交易behavior'] = 1
        card_df.loc[card_df['網路消費次數'] /
                    card_df['交易總次數'] < 0.3, '網路交易behavior'] = 0
