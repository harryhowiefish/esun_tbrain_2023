...
def train(drop_col: list, filename: str):
    ...
    train_dataset = cb.Pool(pd.concat([train, val])[selected_col], pd.concat(
        [train, val])['盜刷註記'], cat_features=cat_col)
    ...
    inference_dataset = cb.Pool(
        public_test[selected_col], cat_features=cat_col)
    ...
    sub.loc[public_test.set_index(
        '交易序號').index, 'pred'] = public_test.set_index('交易序號')['pred']
    ...
    train_dataset = cb.Pool(pd.concat([train, val, public_test])[
                            selected_col], pd.concat([train['盜刷註記'], val['盜刷註記'], gt]), cat_features=cat_col)
    time.sleep(10)
    model = cb.CatBoostClassifier(iterations=150, learning_rate=0.4, depth=16, loss_function='Logloss',
                                  class_weights=[1, 220],
                                  task_type='GPU',
    ...
    private_inference = cb.Pool(
        private_test_1[selected_col], cat_features=cat_col)
    private_test_1['pred'] = model.predict_proba(private_inference)[:, 1]
    sub.loc[private_test_1.set_index(
        '交易序號').index, 'pred'] = private_test_1.set_index('交易序號')['pred']
